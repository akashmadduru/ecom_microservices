...
Companion to `docs/product_service_docs/hld/HLD.md`. Grounded directly in
`services/product_service/src/product_service/{models,repo,schemas,api/routes,seeder,catalog_seeder,_seed_utils,config,deps,main}.py`,
`services/product_service/scripts/generate_seed_data.py`, and
`services/product_service/alembic/versions/0001`–`0006`. The seeding layer's own history
...
entire codebase under `services/product_service/`.
...
`docs/product_service_docs/diagrams/e2e-seller-listing-flow.md`; this is the condensed version:
...
`services/product_service/tests/test-plan.md` — the populated test matrix (91 tests: unit tests
