...
`services/product_service/src/product_service/schemas.py`'s `ProductCreate` and `ProductUpdate`
...
- `DRAFT → SoftDeleted`, `PUBLISHED → SoftDeleted`, and every other `status → SoftDeleted`
  ...
  (`services/product_service/tests/integration/test_soft_delete.py`).
