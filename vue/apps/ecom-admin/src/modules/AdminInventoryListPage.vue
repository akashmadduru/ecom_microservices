<template>
  <div class="space-y-6">
    ...
    >
      <template #action>
        <router-link class="btn btn-primary btn-sm" to="/inventory/new"
          >New inventory record</router-link
        >
      </template>
    </PageHeader>
    ...
    >
      ...
      <template #actions="{ row }">
        <router-link class="btn btn-ghost btn-xs" :to="`/inventory/${row.product_id}`"
          >Manage</router-link
        >
      </template>
    </DataTable>
...
import { listInventory } from 'core/api/InventoryApi'
...
import type { InventoryRecord, InventoryStatus } from 'core/interfaces/inventory'
