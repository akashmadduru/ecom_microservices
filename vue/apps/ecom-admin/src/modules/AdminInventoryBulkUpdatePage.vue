<template>
  <div class="space-y-6">
    <PageHeader
      ...
      title="Bulk update reorder thresholds"
      ...
      <div class="card-body space-y-4">
        ...
        <div v-for="(item, index) in items" :key="index" class="flex flex-wrap items-end gap-3">
          ...
          <label class="form-field">
            ...
            <input
              v-model.number="item.reorder_threshold"
              type="number"
              min="0"
              class="input input-bordered input-sm"
            />
        ...
        <div class="flex gap-3 pt-2">
          ...
          <router-link class="btn btn-outline" to="/inventory">Back to inventory</router-link>
        </div>
      </div>
    </div>
  </div>
...
import type { BulkUpdateItem } from 'core/interfaces/inventory'
...
async function onSubmit() {
  ...
  if (!validItems.length) {
    notificationStore.showToast('Add at least one valid product ID.', 'warning')
    return
  }
  ...
  try {
    ...
    notificationStore.showToast(`Updated ${validItems.length} inventory record(s).`, 'success')
  } catch (err) {
    const apiError = formError.setFromError(err)
    notificationStore.showToast(apiError.message, 'error', 'Bulk update failed')
  } finally {
