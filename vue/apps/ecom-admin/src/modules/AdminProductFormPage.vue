<template>
  <div class="space-y-6">
    <PageHeader
      :eyebrow="isEdit ? 'Admin · Edit product' : 'Admin · New product'"
        ...
        <form v-else class="space-y-4" @submit.prevent="onSubmit" novalidate>
          ...
          <div class="form-row-2">
            ...
            <label class="form-field">
              ...
              <input
                v-model.number="categoryIdValue"
                type="number"
                min="1"
                class="input input-bordered"
              />
          ...
          <label class="form-field">
            ...
            <input
              ...
              class="input input-bordered"
              placeholder="racing wheel, gaming accessories"
            />
          ...
          <div class="flex gap-3">
            ...
            <router-link class="btn btn-outline" to="/products">Cancel</router-link>
          </div>
        </form>
      </div>
    </div>
...
import { useManufacturerStore } from 'core/stores/manufacturer'
...
import { productSchema } from 'core/utils/validators'
import type { CreateProductPayload } from 'core/interfaces/product'
...
async function onSubmit() {
  ...
  try {
    if (isEdit.value && productId.value !== null) {
      ...
      notificationStore.showToast(`Updated ${form.title}.`, 'success')
    } else {
      await productStore.createProduct({ ...buildPayload(), uniq_id: form.uniq_id })
      notificationStore.showToast(`Created ${form.title}.`, 'success')
    }
    router.push('/products')
  } catch (err) {
    const apiError = formError.setFromError(err)
    notificationStore.showToast(apiError.message, 'error', 'Save failed')
  } finally {
...
onMounted(async () => {
  ...
  try {
    ...
    notificationStore.showToast('Failed to load product.', 'error')
  } finally {
