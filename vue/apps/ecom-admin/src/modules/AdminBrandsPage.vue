<template>
  <div class="space-y-6">
    ...
    >
      <template #action>
        <router-link class="btn btn-primary btn-sm" to="/brands/new">New brand</router-link>
      </template>
    </PageHeader>
    ...
    >
      ...
      <template #actions="{ row }">
        <div class="flex justify-end gap-2">
          <router-link class="btn btn-ghost btn-xs" :to="`/brands/${row.id}/edit`"
            >Edit</router-link
          >
          <button
            class="btn btn-ghost btn-xs text-error"
            :disabled="brandStore.mutating"
            @click="askDelete(row)"
          >
            Delete
          </button>
...
import type { Brand } from 'core/interfaces/brand'
