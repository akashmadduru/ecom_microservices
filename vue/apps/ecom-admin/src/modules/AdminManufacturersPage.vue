<template>
  <div class="space-y-6">
    ...
    >
      <template #action>
        <router-link class="btn btn-primary btn-sm" to="/manufacturers/new"
          >New manufacturer</router-link
        >
      </template>
    </PageHeader>
    ...
    <DataTable
      ...
      :columns="columns"
      empty-title="No manufacturers yet"
    ...
    >
      ...
      <template #actions="{ row }">
        <div class="flex justify-end gap-2">
          <router-link class="btn btn-ghost btn-xs" :to="`/manufacturers/${row.id}/edit`"
            >Edit</router-link
          >
          <button
            class="btn btn-ghost btn-xs text-error"
            :disabled="manufacturerStore.mutating"
            @click="askDelete(row)"
          >
            Delete
          </button>
