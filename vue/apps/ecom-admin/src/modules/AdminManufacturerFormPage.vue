<template>
  <div class="space-y-6">
    <PageHeader
      :eyebrow="isEdit ? 'Admin · Edit manufacturer' : 'Admin · New manufacturer'"
      ...
      <div class="card-body">
        ...
        <form v-else class="space-y-4" @submit.prevent="onSubmit" novalidate>
          ...
          <div class="flex gap-3">
            ...
            <router-link class="btn btn-outline" to="/manufacturers">Cancel</router-link>
          </div>
        </form>
      </div>
    </div>
...
import { emailRule, manufacturerSchema } from 'core/utils/validators'
import type { CreateManufacturerPayload } from 'core/interfaces/manufacturer'
...
async function onSubmit() {
  ...
  try {
    ...
    if (isEdit.value && manufacturerId.value !== null) {
      ...
      notificationStore.showToast(`Updated ${form.name}.`, 'success')
    } else {
      await manufacturerStore.createManufacturer(payload)
      notificationStore.showToast(`Created ${form.name}.`, 'success')
    }
...
onMounted(async () => {
  ...
  try {
    ...
    notificationStore.showToast('Failed to load manufacturer.', 'error')
  } finally {
