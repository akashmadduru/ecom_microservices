<template>
  <div class="space-y-6">
    <PageHeader
      eyebrow="Admin · New inventory record"
        ...
        <form class="space-y-4" @submit.prevent="onSubmit" novalidate>
          ...
          <div class="form-row-2">
            <label class="form-field">
              ...
              <span v-if="formError.fieldError('product_id')" class="field-error">{{
                formError.fieldError('product_id')
              }}</span>
            </label>
            <label class="form-field">
          ...
          <label class="form-field">
            ...
            <span v-if="formError.fieldError('warehouse_location')" class="field-error">{{
              formError.fieldError('warehouse_location')
            }}</span>
          </label>
          ...
          <div class="grid gap-4 md:grid-cols-3">
            <label class="form-field">
              ...
              <input
                ...
                class="input input-bordered"
                required
                :class="{ 'input-error': formError.fieldError('available_quantity') }"
              />
              <span v-if="formError.fieldError('available_quantity')" class="field-error">{{
                formError.fieldError('available_quantity')
              }}</span>
            </label>
            <label class="form-field">
              ...
              <input
                ...
                class="input input-bordered"
                required
                :class="{ 'input-error': formError.fieldError('safety_stock') }"
              />
              <span v-if="formError.fieldError('safety_stock')" class="field-error">{{
                formError.fieldError('safety_stock')
              }}</span>
            </label>
            <label class="form-field">
              ...
              <input
                ...
                class="input input-bordered"
                required
                :class="{ 'input-error': formError.fieldError('reorder_threshold') }"
              />
              <span v-if="formError.fieldError('reorder_threshold')" class="field-error">{{
                formError.fieldError('reorder_threshold')
              }}</span>
            </label>
          </div>
          ...
          <div class="flex gap-3">
            <button
              :class="['btn btn-primary', { 'opacity-70': submitting }]"
              type="submit"
              :disabled="submitting"
            >
              {{ submitting ? 'Creating...' : 'Create record' }}
            </button>
            <router-link class="btn btn-outline" to="/inventory">Cancel</router-link>
          </div>
        </form>
      </div>
    </div>
...
import { inventorySchema } from 'core/utils/validators'
import type { CreateInventoryPayload } from 'core/interfaces/inventory'
...
async function onSubmit() {
  ...
  try {
    ...
    notificationStore.showToast(`Inventory created for product #${form.product_id}.`, 'success')
    router.push('/inventory')
  } catch (err) {
    const apiError = formError.setFromError(err)
    notificationStore.showToast(apiError.message, 'error', 'Create failed')
  } finally {
