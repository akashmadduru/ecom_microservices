<template>
  <div class="space-y-6">
    <PageHeader
      :eyebrow="isEdit ? 'Admin · Edit brand' : 'Admin · New brand'"
      ...
      <div class="card-body">
        ...
        <form v-else class="space-y-4" @submit.prevent="onSubmit" novalidate>
          ...
          <div class="flex gap-3">
            ...
            <router-link class="btn btn-outline" to="/brands">Cancel</router-link>
          </div>
        </form>
      </div>
    </div>
...
import { brandSchema } from 'core/utils/validators'
import type { CreateBrandPayload } from 'core/interfaces/brand'
...
onMounted(async () => {
  ...
  try {
    ...
    notificationStore.showToast('Failed to load brand.', 'error')
  } finally {
