  ...
  >
    ...
    <div class="card-body relative z-10 space-y-3">
      ...
      <div class="flex items-center justify-between">
        <div>
          <div class="flex items-baseline gap-2">
            ...
            <p v-if="mrp !== null" class="text-xs text-subtle line-through">
              ₹{{ mrp.toFixed(2) }}
            </p>
          </div>
        ...
        <button
          :class="[
            'btn btn-primary btn-sm transition-all duration-300 hover:scale-[1.02]',
      ...
      <div class="flex gap-2">
        <button
          class="btn btn-outline btn-sm flex-1 transition-all duration-300 hover:scale-[1.01]"
          @click.stop.prevent="viewProduct"
        >
          Details
        </button>
      </div>
...
import { getSellingPrice, getMrp } from 'core/utils/pricing'
...
const isWishlisted = computed(() =>
  ecommerceStore.wishlist.some((item) => item.id === props.product.id),
)
