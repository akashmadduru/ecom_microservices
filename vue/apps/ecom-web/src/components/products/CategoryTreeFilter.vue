    ...
    <div v-for="category in categoryStore.categories" :key="category.id">
      <div class="flex items-center gap-1">
        <button
          ...
          :aria-label="
            expanded.has(category.id) ? `Collapse ${category.name}` : `Expand ${category.name}`
          "
      ...
      <div
        v-if="expanded.has(category.id)"
        :id="`category-children-${category.id}`"
        class="ml-6 mt-1 space-y-1"
      >
        <span
          v-if="categoryStore.isChildrenLoading(category.id)"
          class="loading loading-spinner loading-xs"
        />
        <div
          v-else-if="childrenFailed.has(category.id)"
          class="flex items-center gap-2 text-sm text-error"
        >
          <span>Couldn't load subcategories.</span>
          <button type="button" class="link" @click="toggleExpanded(category.id, true)">
            Retry
          </button>
        </div>
        <label
          v-for="child in categoryStore.getChildren(category.id)"
          :key="child.id"
        ...
        >
          <input
            ...
            @change="selectCategory(child.id)"
          />
