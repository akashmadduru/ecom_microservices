...
import type { SignupPayload } from 'core/interfaces/auth'
...
async function onSubmit() {
  ...
  try {
    ...
    notificationStore.showToast(`Account created for ${user.username}.`, 'success')
    router.push('/')
  } catch (err) {
    const apiError = formError.setFromError(err)
    notificationStore.showToast(apiError.message, 'error', 'Sign up failed')
  } finally {
