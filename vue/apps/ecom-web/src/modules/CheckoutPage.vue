    ...
    <div class="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <div class="space-y-6">
        ...
        <section class="surface-card">
          ...
          <div class="space-y-3">
            <label
              v-for="address in ecommerceStore.addresses"
              :key="address.id"
        ...
        <section class="surface-card">
          ...
          <div class="grid gap-3 md:grid-cols-3">
            <button
              ...
              :class="
                ecommerceStore.paymentMethod === method
                  ? 'border-primary bg-accent/15 font-semibold text-primary'
      ...
      <aside class="surface-card">
        ...
        <div class="mt-4 space-y-3">
          <div
            v-for="item in ecommerceStore.cart"
            :key="item.id"
        ...
        <div class="mt-6 space-y-2 text-sm">
          <div class="flex justify-between">
            <span>Subtotal</span><span>₹{{ ecommerceStore.subtotal }}</span>
          </div>
          <div class="flex justify-between">
            <span>Shipping</span><span>₹{{ ecommerceStore.shipping }}</span>
          </div>
          <div class="flex justify-between">
            <span>Tax</span><span>₹{{ ecommerceStore.tax }}</span>
          </div>
          <div
            class="mt-3 flex justify-between border-t border-base-300 pt-3 text-base font-semibold"
          >
...
const selectedAddress = computed(() =>
  ecommerceStore.addresses.find((address) => address.id === selectedAddressId.value),
)

onMounted(async () => {
  await ecommerceStore.loadInitialData()
  selectedAddressId.value =
    ecommerceStore.shippingAddressId ?? ecommerceStore.addresses[0]?.id ?? null
...
function confirmOrder() {
  if (!selectedAddress.value) {
    notificationStore.showToast('Please choose a delivery address.', 'warning')
    return
  }

  const order = ecommerceStore.placeOrder()
  if (!order) {
    notificationStore.showToast('Your cart is empty.', 'warning')
    return
  }
