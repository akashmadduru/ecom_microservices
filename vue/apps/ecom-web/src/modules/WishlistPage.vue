  ...
  <div class="min-h-screen px-4 py-10 text-base-content">
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <div v-if="ecommerceStore.wishlist.length" class="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        <div v-for="item in ecommerceStore.wishlist" :key="item.id" class="card">
          <figure>
            <AppImage
              :src="item.image_urls"
              alt=""
              img-class="h-48 w-full rounded-[1.1rem] object-cover"
            />
          </figure>
          <div class="card-body">
            ...
            <div class="card-actions justify-between">
              ...
              <button
                ...
                @click="ecommerceStore.removeFromWishlist(item.id)"
              >
                Remove
              </button>
            </div>
