...
import type { InventoryRecord } from 'core/interfaces/inventory'
import type { Manufacturer } from 'core/interfaces/manufacturer'

// ProductPage only calls useRoute() from vue-router; mock it per-test like
// Breadcrumbs.spec.ts does, and stub router-link/AppImage in the mount.
const currentRoute = { value: { params: { id: '1' } } as unknown as RouteLocationNormalizedLoaded }
vi.mock('vue-router', () => ({
  useRoute: () => currentRoute.value,
}))

vi.mock('core/api/ProductsApi', () => ({
  getProduct: vi.fn<(id: number) => Promise<Product>>(),
}))

vi.mock('core/api/InventoryApi', () => ({
  getInventoryByProduct: vi.fn<(productId: number) => Promise<InventoryRecord>>(),
}))

vi.mock('core/api/ManufacturersApi', () => ({
  getManufacturer: vi.fn<(id: number) => Promise<Manufacturer>>(),
}))

import { getProduct } from 'core/api/ProductsApi'
import { getInventoryByProduct } from 'core/api/InventoryApi'
