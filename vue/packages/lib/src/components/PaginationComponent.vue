...
<template>
  <div
    ...
    class="mt-8 flex flex-wrap items-center justify-between gap-3 px-4 py-3"
  >
    <button
    ...
    <div class="flex flex-wrap items-center justify-center gap-2">
      <button
        ...
        :key="page"
        @click="goTo(page)"
        class="btn btn-sm h-10 w-10 border border-base-300 transition-all duration-200"
        :class="
  ...
  <p class="mt-4 text-center text-sm text-muted">
    Page {{ pagination.page }} of {{ pagination.total_pages }} • {{ pagination.total_items }}
    {{ itemLabel }}
  </p>
</template>
